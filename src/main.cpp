...
uint256 hashGenesisBlock("0x000004f6bcb22348082175019b1ab9712d0d21084aeac165862e86b8493b337c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // eponacoin: starting difficulty is 1 / 2^12
static CBigNum bnInitialDifficulty(~uint256(0) >> 22);
...
int64 CTransaction::nMinTxFee = 0.001 * COIN;
/** Fees smaller than this (in satoshi) are considered zero fee (for relaying) */
int64 CTransaction::nMinRelayTxFee = 0.001 * COIN;
...
const string strMessageMagic = "eponacoin Signed Message:\n";
...
{
    // eponacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // eponacoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
	...
	// Mainnet Genesis block:
		...
		const char* pszTimestamp = "epona genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100000000;
	...
	//// debug print
        ...
        assert(block.hashMerkleRoot == uint256("0xdf7203232d1d5550f07079d408f26f42b10cbeed25c89cc801f51ca487c1bea4"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb9, 0xde }; // eponacoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("eponacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("eponacoinMiner : ProcessBlock, block not accepted");
    }
